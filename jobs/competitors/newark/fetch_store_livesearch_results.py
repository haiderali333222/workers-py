...
from .helper import get_manufacturer_page_links, get_product_links_from_plp, MAX_COUNT, add_product_links_to_outputs
...
def get_products_urls_and_store(product_list_page_link):
    try:
        ...
        print(f'product_links: {len(product_links)} and next_pages: {len(next_pages)} for {product_list_page_link}, current count: {len(products_links_outputs)}')
