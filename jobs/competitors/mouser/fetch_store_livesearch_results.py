from utils.helpers.index import url_insert_bulk,error_slack_message, error_slack_message,  get_page_with_scraperapi_from_url
from utils.slack import error_slack_message
from .helper import BASE_URL, COMPETITOR,get_manufacturer_page_links,get_page_with_scraperapi_from_url,get_product_links_from_plp, MAX_COUNT
...
def get_manufacturer_products_links(manufacturer_page_link):
    ...
    count = 0
    try:
        if manufacture_page := get_page_with_scraperapi_from_url(manufacturer_page_link, COMPETITOR, is_ultra_premium=True):
            if all_products_div := manufacture_page.find('div', class_='all_products'):
                if link_input := all_products_div.find(
                    'input'
                ):
                    ...
                    while next_page_exists:
                        ...
                        print(f'product_links: {len(product_links)} next_page: {next_page} count: {count}')
