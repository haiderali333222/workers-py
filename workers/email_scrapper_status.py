...
from celery_once import (
    QueueOnce,  # prevents multiple execution and queuing of celery tasks
)
...
from utils.constants import (
    ...
    SEND_SCRAPERS_STATUS_EMAIL_SUBJECT,
    scraper_status_email_body_template,
)
from utils.cronjob_logs import (
    insert_cron_job_start_status,
    update_cron_job_completed_status,
)
...
def send_scrapper_status():
    try:
        ...
        for entry in last_week_entries:
            ...
            if entry["total"] > 100 and entry["errors"] > entry["success"] and entry["errors"] > chance:
